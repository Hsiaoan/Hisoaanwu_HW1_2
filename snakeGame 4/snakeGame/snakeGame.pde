...
void setup(){
  ...
  portal[0] = new Portal (264, 30, 12, 480, true, 60);
  portal[1] = new Portal (732, 408, 12, 204, true, 60);
  portal[2] = new Portal (352, 312, 640, 360, false, 60);
  portal[3] = new Portal (40, 264, 500, 12, false, 60);
  ...
  for (int i = 0; i < oxygenBox.length; i++) {
    oxygenBox[i] = new PVector(300+i*20, 50);
  }
...
void draw(){
  ...
  if (sceneSwitch) {
    ...
    fill(0, 0, 255);
    rectMode(CORNER);
    rect(pound[0].originPoint.x, pound[0].originPoint.y, pound[0].size.x, pound[0].size.y);
    ...
    if ( abs(snake[0].headLocation.x - pound[0].originPoint.x) <= 6 || abs(snake[0].headLocation.x - (pound[0].originPoint.x + pound[0].size.x)) <= 6 ) {
      if (snake[0].headLocation.y >= pound[0].originPoint.y && snake[0].headLocation.y <= pound[0].originPoint.y + pound[0].size.y) {
        hitBox = true;
      }
    }else if(abs(snake[0].headLocation.y - pound[0].originPoint.y) <= 6 || abs(snake[0].headLocation.y - (pound[0].originPoint.y + pound[0].size.y)) <= 6) {
    ...
    if (hitBox) {
      if (abs(snake[0].headLocation.y - grandPortal[0].positionB.y) <= 6) {
        
        hitPortal = true;
        hitBox = false;
            
        snake[0].headLocation.y = grandPortal[0].positionA.y;
  ...
  /*----------------------------------------------------------------------------------------------------------*/
    ...
    if (hitBox) {
      ...
      if (oxygenFull && !sceneSwitch && abs(snake[0].headLocation.y - grandPortal[0].positionA.y) <= 6) {
    ...
    if (secondCount < 10) {
      secondCount ++;
    }else if (secondCount == 10 && oxygenCount < 20){
      secondCount =0;
      oxygenCount++;
    }
